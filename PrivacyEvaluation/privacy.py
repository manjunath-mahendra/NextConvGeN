...
class AttributeInferenceAttack:
    ...
    def predict_categorical(self, synthetic_data, access_to_attack, feature):
        ...
        # Guard: skip obvious continuous features
        if pd.api.types.is_numeric_dtype(y):
            if y.nunique() > 20:
                print(f"Skipping {feature}: looks continuous, not categorical")
    ...
    def predict_numeric(self, synthetic_data, access_to_attack, feature):
        ...
        reg = DecisionTreeRegressor(random_state=64)
        try:
            reg.fit(X, y)
            ...
            print(f"Regression failed for {feature}, trying classification...")
            acc, f1 = self.predict_categorical(synthetic_data, access_to_attack, feature)
            # Return RMSE as NaN if we fallback to classification
            return np.nan
            ...
            for name in self.column_names:
                if name not in self.QID:
                    if name in self.continuous_features:
                        ...
                        # Let the guard inside predict_categorical decide
                        accuracy, f1 = self.predict_categorical(self.synthetic_data, access_to_attack, name)
                        accuracies.append(accuracy)
                        f1_scores.append(f1)
